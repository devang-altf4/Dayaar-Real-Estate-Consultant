...
export default function AttendancePage() {
  ...
  const { data: statusData, isLoading, refetch } = useQuery({
    ...
    queryFn: () => api.get<any>('/attendance/status'),
  });
  ...
  const checkInMutation = useMutation({
    mutationFn: async () => {
      let coords = { latitude: 28.4595, longitude: 77.0266, accuracy: 10 }; // Office default for testing
      try {
        coords = await getCoordinates();
      } catch (err: any) {
        console.warn('Using default office coordinates for attendance validation in browser environment', err);
      }
  ...
  const checkOutMutation = useMutation({
    mutationFn: async () => {
      let coords = { latitude: 28.4595, longitude: 77.0266, accuracy: 10 };
      try {
        coords = await getCoordinates();
      } catch (err) {}
  ...
  const record = statusData?.attendanceRecord;
        ...
        <div className="p-6 bg-white border border-slate-200 rounded-2xl shadow-xs space-y-5">
          ...
          <div className="p-4 bg-slate-50 border border-slate-100 rounded-xl space-y-2 text-xs text-slate-600">
            <div className="flex justify-between">
              ...
              <b className="text-slate-800">{record ? formatDate(record.checkInTime) : '-'}</b>
            </div>
            <div className="flex justify-between">
              <span>Geofence Verification:</span>
              <span className="text-emerald-700 font-bold flex items-center gap-1">
                <ShieldCheck className="h-3.5 w-3.5" />
                {record?.isWithinGeofence ? 'Verified Inside Office Radius' : 'Pending'}
              </span>
            </div>
            <div className="flex justify-between">
              <span>Check-out Time:</span>
              <b className="text-slate-800">{record?.checkOutTime ? formatDate(record.checkOutTime) : 'Active Shift'}</b>
            </div>
          </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {historyData?.map((h: any) => (
                <tr key={h._id} className="hover:bg-slate-50">
                  ...
                  <td className="py-3 font-mono">{formatDate(h.checkInTime, 'hh:mm a')}</td>
                  <td className="py-3 font-mono">
                    {h.checkOutTime ? formatDate(h.checkOutTime, 'hh:mm a') : 'Active'}
                  </td>
                  <td className="py-3 font-bold text-slate-900">{h.totalWorkMinutes ? `${Math.floor(h.totalWorkMinutes / 60)}h ${h.totalWorkMinutes % 60}m` : '-'}</td>
                  <td className="py-3 font-mono text-amber-700">{h.totalBreakMinutes || 0} mins</td>
                  <td className="py-3">
